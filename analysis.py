...
def evaluate_cluster_stability(X: np.ndarray, n_clusters: int = 5, n_iterations: int = 10) -> Dict:
    """Test stability of clusters by running K-means multiple times"""
    consistency_scores = []
    base_labels = None
    
    for i in range(n_iterations):
        kmeans_temp = KMeans(n_clusters=n_clusters, init='k-means++', random_state=i, n_init=10)
...
def test_kmeans_efficiency(X: np.ndarray, k_range: range = range(2, 11), n_runs: int = 5) -> Dict:
...
def get_cluster_characteristics(X: np.ndarray, labels: np.ndarray, kmeans: KMeans) -> Dict:
...
def statistical_cluster_validation(df: pd.DataFrame, cluster_column: str = 'Cluster') -> Dict:
    ...
    for feature in features:
        cluster_groups = [df[df[cluster_column] == cluster][feature].values for cluster in clusters]
