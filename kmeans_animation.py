...
class KMeansAnimator:
    ...
    def __init__(self, X: np.ndarray, n_clusters: int = 5, max_iter: int = 20, random_state: int = 42):
        self.X = X
    ...
    def update_centroids(self, labels: np.ndarray) -> np.ndarray:
        ...
        centroids = np.array([self.X[labels == k].mean(axis=0) for k in range(self.n_clusters)])
        return centroids
    ...
    def create_2d_animation(self, feature_idx: Tuple[int, int] = (0, 1), 
        ...
        for idx, state in enumerate(self.history):
            ...
            if metrics_idx < len(self.metrics_history):
                ...
                title = (f"Iteration {state['iteration']} - {state['step'].replace('_', ' ').title()}<br>"
    ...
    def create_metrics_animation(self) -> go.Figure:
        ...
        fig.update_layout(
            ...
            yaxis=dict(title=dict(text="Inertia", font=dict(color="blue")), tickfont=dict(color="blue")),
            yaxis2=dict(title=dict(text="Silhouette", font=dict(color="red")), tickfont=dict(color="red"),
                       anchor="x", overlaying="y", side="right"),
            yaxis3=dict(title=dict(text="Davies-Bouldin", font=dict(color="green")), tickfont=dict(color="green"),
                       anchor="free", overlaying="y", side="left", position=0.05),
            yaxis4=dict(title=dict(text="Calinski-Harabasz", font=dict(color="purple")), tickfont=dict(color="purple"),
...
def create_kmeans_animation(X: np.ndarray, n_clusters: int = 5, 
