...
def create_distribution_plots(df: pd.DataFrame) -> go.Figure:
    ...
    # Income distribution
    fig.add_trace(
        go.Histogram(x=df['Annual Income (k$)'], name='Income', marker_color='lightgreen', nbinsx=30),
        row=1, col=2
    )
    
    # Spending Score distribution
    fig.add_trace(
        go.Histogram(x=df['Spending Score (1-100)'], name='Spending Score', marker_color='lightcoral', nbinsx=30),
        row=2, col=1
    )
    ...
    if 'Gender' in df.columns:
        ...
        fig.add_trace(
            go.Bar(x=gender_counts.index, y=gender_counts.values, name='Gender', marker_color='lightsalmon'),
            row=2, col=2
        )
    
    fig.update_layout(height=800, showlegend=False, title_text="Data Distribution Analysis")
    return fig
...
def create_elbow_plots(elbow_data: Dict) -> go.Figure:
    ...
    # WCSS
    fig.add_trace(
        go.Scatter(x=k_range, y=elbow_data['wcss'], mode='lines+markers',
    ...
    # Silhouette Score
    fig.add_trace(
        go.Scatter(x=k_range, y=elbow_data['silhouette_scores'], mode='lines+markers',
    ...
    fig.add_trace(
        go.Scatter(x=k_range, y=elbow_data['ch_scores'], mode='lines+markers',
    ...
    fig.add_trace(
        go.Scatter(x=k_range, y=elbow_data['db_scores'], mode='lines+markers',
                  marker=dict(size=10, color='purple'), line=dict(width=2),
                  name='Davies-Bouldin'),
        row=2, col=2
    )
    ...
    fig.add_annotation(
        x=optimal_sil, y=elbow_data['silhouette_scores'][k_range.index(optimal_sil)],
        text=f"Optimal: {optimal_sil}", showarrow=True, arrowhead=2,
        row=1, col=2
    )
    
    fig.update_layout(height=800, showlegend=False, title_text="Optimal Cluster Selection")
    return fig


def create_2d_cluster_plot(df: pd.DataFrame, x_col: str, y_col: str, 
                           cluster_col: str = 'Cluster', 
                           centroids: np.ndarray = None) -> go.Figure:
        ...
        if centroid_x is not None:
            fig.add_trace(
                go.Scatter(
                    ...
                    mode='markers',
                    marker=dict(size=20, symbol='x', color='red', line=dict(width=2, color='black')),
                    name='Centroids',
                    showlegend=True
                )
            )
...
def create_cluster_characteristics_plot(characteristics: Dict) -> go.Figure:
    ...
    fig = make_subplots(
        rows=1, cols=3,
        subplot_titles=('Cluster Sizes', 'Intra-cluster Distances', 'Feature Importance'),
    ...
    # Cluster sizes
    fig.add_trace(
        go.Bar(x=clusters, y=characteristics['cluster_sizes'], name='Size',
    ...
    fig.add_trace(
        go.Bar(x=clusters, y=characteristics['intra_distances'], name='Distance',
    ...
    fig.add_trace(
        go.Bar(x=features, y=characteristics['feature_importance'], name='Importance',
    ...
    fig.update_layout(height=400, showlegend=False, title_text="Cluster Characteristics")
    return fig
...
def create_efficiency_plot(efficiency_data: Dict) -> go.Figure:
    ...
    # Time vs clusters
    fig.add_trace(
        go.Scatter(x=efficiency_data['k'], y=efficiency_data['time_per_run'],
    ...
    fig.add_trace(
        go.Scatter(x=efficiency_data['k'], y=efficiency_data['iterations'],
    ...
    fig.update_layout(height=400, showlegend=False, title_text="Computational Efficiency")
    return fig
...
def create_cluster_profiles_table(business_data: Dict) -> go.Figure:
    ...
    header_values = ['Cluster', 'Segment Type', 'Size', 'Avg Age', 'Avg Income ($k)', 'Avg Spending Score']
    cell_values = [
        clusters,
    ...
    fig.update_layout(
        title='Cluster Profiles Summary',
        height=300
    )
...
def create_statistical_validation_plot(statistical_data: Dict) -> go.Figure:
    ...
    # F-statistics
    fig.add_trace(
        go.Bar(x=features, y=f_stats, marker_color=colors, name='F-stat'),
        row=1, col=1
    )
    ...
    fig.update_layout(height=400, showlegend=False, title_text="Statistical Validation (ANOVA)")
    return fig
